...
| KI-4 | **`compactHints` has no key binding.** The one-line hint mode exists on the model and in layout math but nothing toggles it yet. | Trivial | Bind a key (e.g. a chord) in `keys.go`/`update_list.go`. |
...
| R6 — two-line hint bar eats space | ⚠️ Partial | Layout supports a 1-line `compactHints` mode, but no toggle key yet (KI-4). |
...
3. **Bind `compactHints` toggle** (KI-4) — closes R6.

### P2
4. **Docker event-driven refresh** (KI-3) — replace/augment polling.
5. **Bulk / multi-select actions** — multi-select in `list.go`, batch via `parallel`-style command fan-out.
6. **Log search & save-to-file** in detail view.
7. **In-container / SSH integration test** — closes R3 fully.
